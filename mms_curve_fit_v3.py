...
'''
def FitSine(timesteps, data, frequencyResolutionHz=0.001):
    ...
    for dim in range(data.shape[-1]):
        ...
        FFTphase = np.arctan(dimFFT.imag, dimFFT.real) / (2*np.pi)
        FFTphase2 = np.arctan2(dimFFT.real, dimFFT.imag) / (2*np.pi)
        ...
        guess_a = [g_amp, g_freq, FFTphase[fBin], g_offset]
        guess_a2 = [g_amp, g_freq, FFTphase2[fBin], g_offset]
